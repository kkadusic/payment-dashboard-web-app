	...
	render() {
		...
		things.thing = things.thing.filter(
			(thing, index, self) =>
				index ===
				self.findIndex(
					(t) => t.text === thing.text && t.value === thing.value
				)
		);
		...
		const columns = [
			{
				...
				onFilter: (value, record) =>
					record.cardNumber.indexOf(value) === 0,
			},
			{
				title: "Merchant",
			...
			},
			{
				...
				onFilter: (value, record) => {
					...
					switch (value) {
						...
						case "lowMedium":
							return (
								price >= lowMediumStart && price < mediumStart
							);
						case "medium":
							return (
								price >= mediumStart && price < upMediumStart
							);
						case "upMedium":
		...
		return (
			<Table
				...
				onExpand={(expanded, render) =>
					this.onTableRowExpand(expanded, render)
				}
				expandable={{
